...
static DWORD DuplicateWinloginToken(DWORD dwSessionId, DWORD dwDesiredAccess, PHANDLE phToken) {
	...
	if (LookupPrivilegeValue(NULL, SE_TCB_NAME, &ps.Privilege[0].Luid)) {
		...
		if (INVALID_HANDLE_VALUE != hSnapshot) {
			...
			dbgout("Scanning winlogon process...\n");
	...
	if (CheckForUIAccess(&dwErr, (DWORD*)&fUIAccess)) {
		if (fUIAccess) {
			...
			if (ERROR_SUCCESS == dwErr) {
				...
				GetStartupInfoA(&si);
				if (CreateProcessAsUserA(hTokenUIAccess, NULL, (LPSTR)StrConnect(GetCommandLineA()," --int_uiaccess_token_granted"), NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
